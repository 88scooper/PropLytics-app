...
const BaselineForecast = ({ property, assumptions }) => {
  ...
  const showLeftAxis = visibleMetrics.netCashFlow;
  const showRightAxis = visibleMetrics.mortgageBalance || visibleMetrics.equity;
  ...
  const renderCustomLegend = () => {
    const allMetrics = [
      ...
      { key: 'equity', name: 'Total Equity', color: '#3b82f6' },
    ];
  ...
  const metricCards = [
    {
      ...
      year10Value: forecastData[9]?.netCashFlow || 0,
    },
    {
      key: 'mortgageBalance',
      ...
      year10Value: forecastData[9]?.mortgageBalance || 0,
    },
    {
      key: 'equity',
      label: 'Total Equity',
      color: 'blue',
      year1Value: forecastData[0]?.equity || 0,
      year10Value: forecastData[9]?.equity || 0,
    },
  ];
          ...
          >
            ...
            {showRightAxis && (
              <YAxis
                ...
                label={{ value: 'Balance/Equity ($)', angle: 90, position: 'insideRight' }}
                className="text-gray-600 dark:text-gray-400"
              />
            )}
